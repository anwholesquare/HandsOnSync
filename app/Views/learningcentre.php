...
<body>
    ...
    <script defer type="module">
        ...
        async function loadWordList() {
            wordList = await fetch('../../../learningList').then(res => res.text()).then(text => text.split('\n'));
            setNewWordToGuess();
        }
        ...
        function checkWordGuess() {
            ...
            if (userGuess == wordToGuess) {
                ...
                setNewWordToGuess();
                setTimeout(() => showFeedbackMessage('New Letter Generated'), 500);
            }
        }
            ...
            if (!currentlyPlaying) {
                switch (event.ctrlKey && event.key) {
                    case 'n': // handle word generation
                        setNewWordToGuess();
                        showFeedbackMessage('New Letter Generated');
                        break;
                    case ']': // increase speed
